...
class DuneQueryClient:
    ...
    def execute_query(
        self, sql: str, query_params: List[Dict[str, Any]] = None
    ...
    def create_and_execute_query(
        ...
        try:
            # 处理查询参数
            parameters = (
                self._process_query_parameters(query_params) if query_params else []
            )
        ...
        for param in query_params:
            ...
            try:
                if param_type == "text":
                    ...
                    parameters.append(
                        QueryParameter.number_type(name=name, value=float(value))
                    )
    ...
    def _create_query(
        self, sql: str, parameters: List[QueryParameter]
    ) -> Union[QueryBase, str]:
        """创建新查询"""
        try:
            query = self.client.create_query(
                name="AI生成查询", query_sql=sql, params=parameters, is_private=False
            )
    ...
    def _extract_detailed_error(self, error_msg: str) -> str:
        ...
        try:
            ...
            if "ExecutionState.FAILED" in error_msg:
                ...
                execution_id = (
                    execution_id_match.group(1) if execution_id_match else "未知"
                )
